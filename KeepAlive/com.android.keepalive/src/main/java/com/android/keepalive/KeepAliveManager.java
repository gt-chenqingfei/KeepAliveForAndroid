...
public class KeepAliveManager implements IKeepAliveManager {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                JobInfo.Builder builder = new JobInfo.Builder(job,
                        new ComponentName(KeepAliveApplication.getApp(), JobSchedulerService.class));
                builder.setPeriodic(JOB_SCHEDULER_INTERVAL_MILLIS);
                builder.setPersisted(true);

                JobScheduler jobScheduler = (JobScheduler) KeepAliveApplication.getApp().
    ...
    public static void addAccount() {
        try {
            SyncAccountUtils.createSyncAccount(KeepAliveApplication.getApp());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
