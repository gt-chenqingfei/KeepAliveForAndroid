...
JNIEXPORT void JNICALL Java_com_android_keepalive_natives_NativeDaemonAPI20_doDaemon(JNIEnv *env, jobject jobj, jstring pkgName, jstring svcName, jstring daemonPath){
	if(pkgName == NULL || svcName == NULL || daemonPath == NULL){
		LOGE("native doDaemon parameters cannot be NULL !");
		return ;
	}

	LOGI("Native daemonV20 doDaemon!");
